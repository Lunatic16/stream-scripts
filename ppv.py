...
def _build_api_chain(requested_base: str) -> list[str]:
    ...
    The user's requested base goes first. Then any ALT_API_BASES entries
    that aren't already in the chain. Finally, dynamic fallbacks derived
    from ``API_DOMAINS`` (in case a brand-new domain mirrors the API but
    isn't in ALT_API_BASES yet — cheap insurance).
    """
    seen: set[str] = set()
    chain: list[str] = []
...
class _AllBasesFailed(RuntimeError):
    ...
    def __init__(self, attempted: list[str], last_error: str):
        super().__init__(
            f"all {len(attempted)} API base(s) failed; "
...
class PPVClient:
    ...
    def _unrecoverable(exc: BaseException) -> bool:
        """A LookupError-typed exception is a real 'not found' result; we
        must NOT failover past it (the event genuinely doesn't exist).
        Anything else (network, HTTP 5xx, malformed JSON, success=false)
        means "this base is broken or unreachable" and we should try the
        next one.
        """
        return isinstance(exc, LookupError)

    def _with_failover(self, op_name: str, call) -> tuple[Any, str]:
        """Try ``call(self.client, base)`` against each base in api_chain in
        order. Returns (result, used_base) on success. Raises
        _AllBasesFailed if every base errors out with a recoverable failure,
        or re-raises the unrecoverable exception from the first base that
        produces one.
        """
        attempted: list[str] = []
        ...
        for base in self.api_chain:
            ...
            try:
                ...
                if self._unrecoverable(e):
                    # Domain-level "this event does not exist" — don't skip
                    # hosts, this same answer would come from every mirror.
                    raise
                last_err = e
                # continue to next base
                continue
        # All bases failed with recoverable errors.
    ...
    def index(self) -> list[dict[str, Any]]:
        """Returns the flat streams list from /api/streams, walking the
        failover chain if the primary base fails.
        """
    ...
    def event(self, uri_path: str) -> dict[str, Any]:
        """Returns the `data` object from /api/streams/<uri-path>, walking
        the failover chain if the primary base fails. A 404 (LookupError)
        short-circuits every base — it's a real not-found.
        """
...
def run(api_base: str, show_default: bool, use_color: bool) -> int:
    ...
    def announce_failover(prev: str, nxt: str, why: str) -> None:
        if not sys.stdout.isatty():
            sys.stdout.write(f"  {prev} failed ({why}); retrying with {nxt}\n")
            sys.stdout.flush()
            return
        msg = (
            f"\r  {c.amber}↻{c.reset}  {c.dim}{prev}{c.reset} "
            f"unreachable ({why}) → trying {c.sky}{nxt}{c.reset}\033[K"
        )
    ...
    used_base_box: list[str] = [chain[0]]  # mutated when failover lands

    with PPVClient(api_base=api_base) as client:

        # ── fetch event index (with failover) ────────────────────────────────
        ...
        try:
            ...
            for base in chain:
                attempted.append(base)
                if len(attempted) > 1:
                    announce_failover(attempted[-2], base, "transport / API error")
                client.api_base = base  # retarget for this attempt
                try:
                    index = fetch_with_spinner(
                        f"fetching event index [{base}]",
                        client.index,
                        c,
                    )
                    used_base_box[0] = base
                    break
                except LookupError:
                    raise
                except Exception as e:
        ...
        # If we landed somewhere other than the default, acknowledge it on stderr.
        if used_base_box[0] != chain[0]:
            sys.stderr.write(
                f"  {c.amber}↻{c.reset}  {c.dim}using fallback {used_base_box[0]}{c.reset}\n"
            )
        ...
        # Start the detail-call chain at the same base that answered for
        # the index — the surviving mirror is most likely to also have
        # the per-event record.
        detail_attempted: list[str] = []
        detail_last_err: BaseException | None = None
        detail: dict[str, Any] | None = None
        start_idx = chain.index(used_base_box[0]) if used_base_box[0] in chain else 0
        prior_base = used_base_box[0]
        for base in chain[start_idx:]:
            detail_attempted.append(base)
            client.api_base = base
            try:
                detail = fetch_with_spinner(
                    f"fetching detail for \"{chosen.name}\" [{base}]",
                ...
                if detail_attempted[-1] != chain[-1]:
                    announce_failover(prior_base, chain[min(chain.index(base) + 1, len(chain) - 1)], "transport / API error")
                detail_last_err = e
                prior_base = base
                continue
        if detail is None and detail_last_err is not None:
            sys.stderr.write(f"\n{c.warn}  ⚠  event detail failed: {detail_last_err}{c.reset}\n")
