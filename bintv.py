#!/usr/bin/env python3
"""
bintv.py — browse bintv.cc event index in your terminal and emit the
stream URLs for any event and any of its streams.

This version adds a post-stream-selection action menu:
...
Usage:
    python bintv.py                    # interactive event + stream picker
    python bintv.py --raw              # disable ANSI colors
    python bintv.py --category Soccer  # filter to specific category
    python bintv.py --live-only        # show only live events
    python bintv.py --list             # plain text list (non-interactive)
    python bintv.py --json             # JSON output for scripting
...
def run(
    ...
    while True:
        ...
        while True:
            ...
            if idx2 is None:
                # Esc / cancel from stream picker -> back to event list
                break
            ...
            while True:
                ...
                # Esc from action menu returns to stream list.
                if action is None:
                    break
                ...
                if action == "browser":
                    ...
                    print_stream_details(stream, chosen.title, c)
                    pause(c)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="bintv.py",
        description="Browse bintv.cc events from the terminal.",
    )
